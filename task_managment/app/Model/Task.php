...
function insert_task($conn, $data){
	$sql = "INSERT INTO tasks (title, description, assigned_to, deadline, hours) VALUES(?,?,?,?,?)";
...
function get_all_tasks_due_today($conn){
	$sql = "SELECT * FROM tasks WHERE deadline = CURDATE() AND status != 'completed' ORDER BY id DESC";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function count_tasks_due_today($conn){
	$sql = "SELECT id FROM tasks WHERE deadline = CURDATE() AND status != 'completed'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function get_all_tasks_NoDeadline($conn){
	$sql = "SELECT * FROM tasks WHERE status != 'completed' AND (deadline IS NULL OR deadline = '0000-00-00') ORDER BY id DESC";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function count_tasks_NoDeadline($conn){
	$sql = "SELECT id FROM tasks WHERE status != 'completed' AND (deadline IS NULL OR deadline = '0000-00-00')";
	$stmt = $conn->prepare($sql);
	$stmt->execute([]);
...
function update_task($conn, $data){
	$sql = "UPDATE tasks SET title=?, description=?, assigned_to=?, deadline=?, hours=? WHERE id=?";
	$stmt = $conn->prepare($sql);
	$stmt->execute($data);
}
...
function count_my_tasks_overdue($conn, $id){
	$sql = "SELECT id FROM tasks WHERE deadline < CURDATE() AND status != 'completed' AND assigned_to=? AND deadline != '0000-00-00'";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
...
function count_my_tasks_NoDeadline($conn, $id){
	$sql = "SELECT id FROM tasks WHERE assigned_to=? AND status != 'completed' AND (deadline IS NULL OR deadline = '0000-00-00')";
	$stmt = $conn->prepare($sql);
	$stmt->execute([$id]);
